...
echo -e "$GREEN**********************************************************************************$RESET"
sleep 1
echo -e "  				This Script for Automate  disnable deletion protection for an instance **$RESET"              
sleep 5
...
echo -e "                        \t   $RED Hi $USER $RESET"
echo ""
read -p "Please enter project you need to disnable deletion protection under it : " PROJECTT;
gcloud config set project "$PROJECTT"
echo "Project set to "$PROJECTT""
while IFS=, read NAME ZONE; do

	gcloud compute instances update "$NAME" --no-deletion-protection --zone="$ZONE" --project="$PROJECTT"
